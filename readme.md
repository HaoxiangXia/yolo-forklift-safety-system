...
使用 MaixCAM Pro 和 YOLO11n 模型，实现实时“人”检测，并通过 UART 串口向 ESP32 发送状态信号。

---

### 功能简介

- 实时检测摄像头画面中的人（person 类）
- 连续 N 帧检测到人时，发送 `'1'` 给 ESP32
- 连续 M 帧未检测到人时，发送 `'0'` 给 ESP32
- 串口仅在 **状态切换** 时发送，避免重复发送
- 适用于嵌入式边缘 AI 场景
...
1. MaixCAM Pro 获取实时图像

2. 使用 YOLO11n 模型进行 person 检测

3. 使用状态机逻辑判断连续帧情况

4. 根据状态变化通过UART串口向 ESP32 发送 '1' 或 '0'

5. ESP32 接收信号进行后续控制（LED、蜂鸣器）
...
```text
src/
├── config.py      # 全局配置中心 (模型路径、检测阈值、逻辑常量)
├── detector.py    # YOLO 推理模块 (封装模型加载与推理逻辑)
├── logic.py       # 业务逻辑模块 (状态机、人员入侵判定逻辑)
├── logger.py      # 性能与日志模块 (FPS 统计、日志文件写入)
└── main.py        # 程序入口 (整合各模块、相机流处理、UI 显示)
```
